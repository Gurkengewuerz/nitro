        ...
        <NitroCardView className="nitro-pet-package no-resize" theme="primary-slim">
          ...
          <NitroCardContentView>
            <Flex className="pet-package-container-top p-3">
              ...
              <div className="m-2">
                <Text variant="white" className="package-text-big">
              ...
              <Column gap={1}>
                <Flex alignItems="center" className="bg-white rounded py-1 px-2 input-pet-package-container">
                  <input
                    ...
                    maxLength={GetConfiguration("pet.package.name.max.length")}
                    placeholder={objectType === "gnome_box" ? LocalizeText("widgets.gnomepackage.name.select") : LocalizeText("widgets.petpackage.name.title")}
                    value={petName}
                    onChange={event => onChangePetName(event.target.value)}
                  />
                  <div className="package-pencil-image flex-shrink-0 small fa-icon"></div>
                </Flex>
                {errorResult.length > 0 && <Base className="invalid-feedback d-block m-0">{errorResult}</Base>}
                <Flex className="mt-2" gap={5} display="flex" justifyContent="center" alignItems="center">
                  ...
                  <Button variant={petName.length < 3 ? "danger" : "success"} disabled={petName.length < 3} onClick={() => onConfirm()}>
