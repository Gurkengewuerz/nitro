                  ...
                  <Base fullWidth overflow="hidden" position="relative" className="bg-light-dark rounded">
                    <Flex fit center position="absolute">
                      <Text variant="white" small>
                        {avatarInfo.dead
                          ? "00:00:00"
                          : ConvertSeconds(remainingTimeToLive == 0 ? avatarInfo.remainingTimeToLive : remainingTimeToLive).split(":")[1] +
                            ":" +
                            ConvertSeconds(remainingTimeToLive == null || remainingTimeToLive == undefined ? 0 : remainingTimeToLive).split(":")[2] +
                            ":" +
                            ConvertSeconds(remainingTimeToLive == null || remainingTimeToLive == undefined ? 0 : remainingTimeToLive).split(":")[3]}
                      </Text>
                    </Flex>
                    <Base
                      className="bg-success rounded pet-stats"
                      style={{width: avatarInfo.dead ? "0" : Math.round((avatarInfo.maximumTimeToLive * 100) / remainingTimeToLive).toString()}}
                    />
                  </Base>
                </Column>
