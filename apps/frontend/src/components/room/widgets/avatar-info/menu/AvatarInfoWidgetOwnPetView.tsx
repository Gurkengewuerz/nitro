import {PetRespectComposer, PetType, RoomObjectCategory, RoomObjectType, RoomObjectVariable, RoomUnitGiveHandItemPetComposer} from "@nitro/renderer";
    ...
    <ContextMenuView objectId={avatarInfo.roomIndex} category={RoomObjectCategory.UNIT} userType={RoomObjectType.PET} onClose={onClose} collapsable={true}>
      ...
      {mode === MODE_NORMAL && (
        <>
          ...
          {avatarInfo.petType === PetType.HORSE && (
            <ContextMenuListItemView onClick={event => processAction("buy_saddle")}>{LocalizeText("infostand.button.buy_saddle")}</ContextMenuListItemView>
          )}
      ...
      {mode === MODE_SADDLED_UP && (
        <>
          ...
          <ContextMenuListItemView onClick={event => processAction("train")}>{LocalizeText("infostand.button.train")}</ContextMenuListItemView>
          <ContextMenuListItemView onClick={event => processAction("pick_up")}>{LocalizeText("infostand.button.pickup")}</ContextMenuListItemView>
          <ContextMenuListItemView onClick={event => processAction("saddle_off")}>{LocalizeText("infostand.button.saddleoff")}</ContextMenuListItemView>
        </>
      )}
      {mode === MODE_RIDING && (
        <>
          <ContextMenuListItemView onClick={event => processAction("dismount")}>{LocalizeText("infostand.button.dismount")}</ContextMenuListItemView>
          {petRespectRemaining > 0 && (
            <ContextMenuListItemView onClick={event => processAction("respect")}>
              {LocalizeText("infostand.button.petrespect", ["count"], [petRespectRemaining.toString()])}
            </ContextMenuListItemView>
      ...
      {mode === MODE_MONSTER_PLANT && (
        <>
          <ContextMenuListItemView onClick={event => processAction("pick_up")}>{LocalizeText("infostand.button.pickup")}</ContextMenuListItemView>
          {avatarInfo.dead && (
            <ContextMenuListItemView onClick={event => processAction("revive")}>{LocalizeText("infostand.button.revive")}</ContextMenuListItemView>
          )}
          {roomSession.isRoomOwner && (
            <ContextMenuListItemView onClick={event => processAction("compost")}>{LocalizeText("infostand.button.compost")}</ContextMenuListItemView>
          )}
      ...
      {canGiveHandItem && (
        <ContextMenuListItemView onClick={event => processAction("pass_hand_item")}>{LocalizeText("infostand.button.pass_hand_item")}</ContextMenuListItemView>
      )}
    </ContextMenuView>
  );
};
