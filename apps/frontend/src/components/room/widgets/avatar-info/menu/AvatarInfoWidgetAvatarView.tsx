  ...
  return (
    <ContextMenuView objectId={avatarInfo.roomIndex} category={RoomObjectCategory.UNIT} userType={avatarInfo.userType} onClose={onClose} collapsable={true}>
      ...
      {mode === MODE_NORMAL && (
        <>
          {canRequestFriend(avatarInfo.webID) && (
            <ContextMenuListItemView onClick={event => processAction("friend")}>{LocalizeText("infostand.button.friend")}</ContextMenuListItemView>
          )}
          ...
          {!avatarInfo.isIgnored && (
            <ContextMenuListItemView onClick={event => processAction("ignore")}>{LocalizeText("infostand.button.ignore")}</ContextMenuListItemView>
          )}
          {avatarInfo.isIgnored && (
            <ContextMenuListItemView onClick={event => processAction("unignore")}>{LocalizeText("infostand.button.unignore")}</ContextMenuListItemView>
          )}
          ...
          {canGiveHandItem && (
            <ContextMenuListItemView onClick={event => processAction("pass_hand_item")}>{LocalizeText("avatar.widget.pass_hand_item")}</ContextMenuListItemView>
          )}
        </>
      )}
      {mode === MODE_MODERATE && (
        <>
          ...
          {isShowGiveRights && (
            <ContextMenuListItemView onClick={event => processAction("give_rights")}>{LocalizeText("infostand.button.giverights")}</ContextMenuListItemView>
          )}
          {isShowRemoveRights && (
            <ContextMenuListItemView onClick={event => processAction("remove_rights")}>{LocalizeText("infostand.button.removerights")}</ContextMenuListItemView>
          )}
      ...
      {mode === MODE_MODERATE_BAN && (
        <>
          <ContextMenuListItemView onClick={event => processAction("ban_hour")}>{LocalizeText("infostand.button.ban_hour")}</ContextMenuListItemView>
          ...
          <ContextMenuListItemView onClick={event => processAction("perm_ban")}>{LocalizeText("infostand.button.perm_ban")}</ContextMenuListItemView>
      ...
      {mode === MODE_MODERATE_MUTE && (
        <>
          <ContextMenuListItemView onClick={event => processAction("mute_2min")}>{LocalizeText("infostand.button.mute_2min")}</ContextMenuListItemView>
          <ContextMenuListItemView onClick={event => processAction("mute_5min")}>{LocalizeText("infostand.button.mute_5min")}</ContextMenuListItemView>
          <ContextMenuListItemView onClick={event => processAction("mute_10min")}>{LocalizeText("infostand.button.mute_10min")}</ContextMenuListItemView>
          <ContextMenuListItemView onClick={event => processAction("back_moderate")}>
            <FaChevronLeft className="left fa-icon" />
            {LocalizeText("generic.back")}
          </ContextMenuListItemView>
      ...
      {mode === MODE_AMBASSADOR && (
        <>
          <ContextMenuListItemView onClick={event => processAction("ambassador_alert")}>{LocalizeText("infostand.button.alert")}</ContextMenuListItemView>
          <ContextMenuListItemView onClick={event => processAction("ambassador_kick")}>{LocalizeText("infostand.button.kick")}</ContextMenuListItemView>
      ...
      {mode === MODE_RELATIONSHIP && (
        <>
          ...
          <ContextMenuListItemView onClick={event => processAction("rship_none")}>{LocalizeText("avatar.widget.clear_relationship")}</ContextMenuListItemView>
          <ContextMenuListItemView onClick={event => processAction("back")}>
            <FaChevronLeft className="left fa-icon" />
            {LocalizeText("generic.back")}
          </ContextMenuListItemView>
