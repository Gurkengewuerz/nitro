      ...
      <NitroCardContentView className="text-black">
        <Flex alignItems="center" justifyContent="center" gap={2}>
          <Grid columnCount={2}>
            ...
            <Text variant="black" small wrap>
              {petData.name}
            </Text>
          </Grid>
        </Flex>
        <Grid columnCount={2}>
          {petTrainInformation.commands &&
            petTrainInformation.commands.length > 0 &&
            petTrainInformation.commands.map((command, index) => (
              <Button
                key={index}
                disabled={!petTrainInformation.enabledCommands.includes(command)}
                onClick={() => processPetAction(petData.name, LocalizeText(`pet.command.${command}`))}
              >
                {LocalizeText(`pet.command.${command}`)}
              </Button>
            ))}
        </Grid>
