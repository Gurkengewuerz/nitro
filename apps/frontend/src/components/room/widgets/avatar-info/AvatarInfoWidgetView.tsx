      ...
      switch (avatarInfo.type) {
        ...
        case AvatarInfoUser.PEER: {
          ...
          if (info.isOwnUser) {
            ...
            return (
              <AvatarInfoWidgetOwnAvatarView avatarInfo={info} isDancing={isDancing} setIsDecorating={setIsDecorating} onClose={() => setAvatarInfo(null)} />
            );
          }
    ...
    <>
      ...
      {productBubbles.length > 0 &&
        productBubbles.map((item, index) => {
          return (
            <AvatarInfoUseProductView key={item.id} item={item} updateConfirmingProduct={updateConfirmingProduct} onClose={() => removeProductBubble(index)} />
          );
        })}
