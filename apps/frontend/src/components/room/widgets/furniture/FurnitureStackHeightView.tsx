  ...
  return (
    <NitroCardView className="nitro-widget-custom-stack-height" theme="primary-slim">
      ...
      <NitroCardContentView justifyContent="between">
        ...
        <Column gap={1}>
          <Button onClick={event => SendMessageComposer(new FurnitureStackHeightComposer(objectId, -100))}>{LocalizeText("furniture.above.stack")}</Button>
          <Button onClick={event => SendMessageComposer(new FurnitureStackHeightComposer(objectId, 0))}>{LocalizeText("furniture.floor.level")}</Button>
        </Column>
