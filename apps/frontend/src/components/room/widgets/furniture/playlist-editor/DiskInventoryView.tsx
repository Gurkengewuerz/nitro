import {IAdvancedMap, MusicPriorities} from "@nitro/renderer";
          ...
          {diskInventory &&
            diskInventory.getKeys().map((key, index) => {
              ...
              return (
                ...
                >
                  <div className="disk-image flex-shrink-0 mb-n2" style={{backgroundColor: GetDiskColor(songInfo?.songData)}}></div>
                  <Text truncate fullWidth className="text-center">
                    {songInfo?.name}
                  </Text>
                  {selectedItem === index && (
