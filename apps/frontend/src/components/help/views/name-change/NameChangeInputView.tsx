...
export const NameChangeInputView: FC<NameChangeLayoutViewProps> = props => {
  ...
  return (
    <div className="d-flex flex-column gap-3 h-100">
      ...
      {suggestions && (
        <div className="d-flex flex-column gap-2">
          {suggestions.map((suggestion, index) => (
            <div key={index} className="col bg-muted rounded p-1 cursor-pointer" onClick={() => handleUsernameChange(suggestion)}>
              {suggestion}
            </div>
          ))}
        </div>
      ...
      <div className="d-flex gap-2">
        <button className="btn btn-success w-100" disabled={!canProceed} onClick={() => onAction("confirmation", newUsername)}>
          {LocalizeText("tutorial.name_change.pick")}
        </button>
        <button className="btn btn-primary w-100" onClick={() => onAction("close")}>
          {LocalizeText("cancel")}
        </button>
      </div>
    </div>
