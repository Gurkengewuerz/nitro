  ...
  return (
    <>
      ...
      <Column gap={1}>
        ...
        <Button onClick={() => DispatchUiEvent(new GuideToolEvent(GuideToolEvent.CREATE_HELP_REQUEST))} disabled={!GetConfiguration("guides.enabled")}>
          {LocalizeText("help.main.help.title")}
        </Button>
        <Button disabled={true}>{LocalizeText("help.main.self.tips.title")}</Button>
      </Column>
      <Button variant="link" textColor="black" onClick={() => SendMessageComposer(new GetCfhStatusMessageComposer(false))}>
