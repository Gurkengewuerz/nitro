          ...
          {membersData.result.map((member, index) => {
            return (
              <Flex key={index} gap={2} alignItems="center" overflow="hidden" className="member-list-item bg-white rounded p-2">
                ...
                <Column grow gap={1}>
                  <Text bold small pointer onClick={event => GetUserProfile(member.id)}>
                    {member.name}
                  </Text>
                  {member.rank !== GroupRank.REQUESTED && (
                    <Text small italics variant="muted">
                ...
                <Column gap={1}>
                  {member.rank !== GroupRank.REQUESTED && (
                    <Flex center>
                      <Base
                        pointer={membersData.admin}
                        className={`icon icon-group-small-${
                          member.rank === GroupRank.OWNER
                            ? "owner"
                            : member.rank === GroupRank.ADMIN
                            ? "admin"
                            : membersData.admin && member.rank === GroupRank.MEMBER
                            ? "not-admin"
                            : ""
                        }`}
