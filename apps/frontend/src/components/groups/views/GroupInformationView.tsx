    ...
    <Grid overflow={overflow} {...rest}>
      <Column center size={3} overflow="hidden">
        ...
        <Column alignItems="center" gap={1}>
          <Text small underline pointer onClick={() => handleAction("members")}>
          ...
          {groupInformation.pendingRequestsCount > 0 && (
            <Text small underline pointer onClick={() => handleAction("members_pending")}>
              {LocalizeText("group.pendingmembercount", ["amount"], [groupInformation.pendingRequestsCount.toString()])}
            </Text>
          )}
          {groupInformation.isOwner && (
            <Text small underline pointer onClick={() => handleAction("manage")}>
          ...
          <Column gap={1}>
            <Flex alignItems="center" gap={2}>
              ...
              <Flex gap={1}>
                ...
                />
                {groupInformation.canMembersDecorate && <i className="icon icon-group-decorate" title={LocalizeText("group.memberscandecorate")} />}
              </Flex>
            </Flex>
            <Text small>{LocalizeText("group.created", ["date", "owner"], [groupInformation.createdAt, groupInformation.ownerName])}</Text>
          </Column>
          <Text small overflow="auto" className="group-description">
            {groupInformation.description}
          </Text>
        </Column>
        <Column>
          <Column gap={1}>
            <Text small underline pointer onClick={() => handleAction("homeroom")}>
              {LocalizeText("group.linktobase")}
            </Text>
            <Text small underline pointer onClick={() => handleAction("furniture")}>
              {LocalizeText("group.buyfurni")}
            </Text>
            <Text small underline pointer onClick={() => handleAction("popular_groups")}>
