...
const TABS: string[] = ["Open Issues", "My Issues", "Picked Issues"];
...
export const ModToolsTicketsView: FC<ModToolsTicketsViewProps> = props => {
  ...
  const CurrentTabComponent = () => {
    switch (currentTab) {
      ...
      case 1:
        return <ModToolsMyIssuesTabView myIssues={myIssues} handleIssue={handleIssue} />;
      case 2:
    ...
    <>
      <NitroCardView className="nitro-mod-tools-tickets">
        ...
        <NitroCardTabsView>
          {TABS.map((tab, index) => {
            return (
              <NitroCardTabsItemView key={index} isActive={currentTab === index} onClick={event => setCurrentTab(index)}>
                {tab}
