...
export const ChatHistoryView: FC<{}> = props =>
{
    ...
    const filteredChatHistory = useMemo(() =>
    {
        ...
        else
        {
            ...
            result = chatHistory.filter(entry => ((entry.message && entry.message.toLowerCase().includes(text))) || (entry.name && entry.name.toLowerCase().includes(text)));
        }
    ...
    return (
        <NitroCardView uniqueKey="chat-history" className="nitro-chat-history" theme="primary-slim">
            ...
            <NitroCardContentView overflow="hidden" gap={ 2 }>
                ...
                <InfiniteScroll rows={ filteredChatHistory } rowRender={ row =>
                {
                    return (
