  ...
  return (
    <Flex gap={2}>
      ...
      <Column>
        ...
        <Flex gap={1}>
          {userProfile.isOnline && <i className="icon icon-pf-online" />}
          {!userProfile.isOnline && <i className="icon icon-pf-offline" />}
          <Flex alignItems="center" gap={1}>
            ...
            {!canSendFriendRequest && (
              <>
                ...
                {isOwnProfile && <Text>{LocalizeText("extendedprofile.me")}</Text>}
                {userProfile.isMyFriend && <Text>{LocalizeText("extendedprofile.friend")}</Text>}
                {(requestSent || userProfile.requestSent) && <Text>{LocalizeText("extendedprofile.friendrequestsent")}</Text>}
              </>
            )}
          </Flex>
        </Flex>
