import {AvatarAction, IQuestion, RoomSessionWordQuizEvent} from "@nitro/renderer";
...
const useWordQuizWidgetState = () => {
  ...
  useRoomSessionManagerEvent<RoomSessionWordQuizEvent>(RoomSessionWordQuizEvent.ANSWERED, event => {
    ...
    GetRoomEngine().updateRoomObjectUserGesture(
      roomSession.roomId,
      userData.roomIndex,
      AvatarAction.getGestureId(event.value === "0" ? AvatarAction.GESTURE_SAD : AvatarAction.GESTURE_SMILE)
    );
  });
